...
//   submission via POST
// * Make all y.rest(...) calls time out after 10 seconds (10000 ms)
...
function handleRedirect(request) {
  ...
  while (request.status == 301 || request.status == 302) {
    ...
    request = y.rest("http://www.cboe.com" + request.headers.location)
      .timeout(10000).accept("text/html");
...
var request = y.rest(url).timeout(10000).followRedirects(false).accept("text/html").get();
...
var params = "";
var submit = y.rest(action).timeout(10000).accept("text/html").followRedirects(false);
if (cookie != "") {
